import {cashSummary} from './expense-math.js';
...
const tables=['fractions','condominium_members','issues','notices','documents','suppliers','equipment','maintenance','obligations','obligation_inspections','assemblies','polls','periodic_services','periodic_service_visits','fraction_charges','fraction_payments','payment_allocations','obligation_checklist_items','payment_proofs','service_contracts','condominium_expenses','condominium_balance_settings'];
...
export function reportMarkup(condo,d){
 ...
 const cash=cashSummary(d.fraction_payments,d.condominium_expenses||[],d.condominium_balance_settings?.[0],'0001-01-01',`${part('year')}-${part('month')}-${part('day')}`);
 ...
 <section class="cr-section"><h3>Resumo financeiro</h3><div class="cr-stats">${[['Emitido (sem anuladas)',total.charged],['Recebimentos',total.receipts],['Por liquidar',total.outstanding],['Em atraso',total.overdue],['Recebido por alocar',total.unallocated]].map(([k,v])=>`<div><strong>${money(v)}</strong><span>${k}</span></div>`).join('')}</div><p class="cr-muted">Valores de quotas e pagamentos registados. O balanço de caixa e as despesas constam abaixo; não existe conciliação bancária automática. Os montantes por liquidar são calculados pelas alocações dos pagamentos.</p></section>
 <section class="cr-section"><h3>Balanço de caixa</h3><p>${cash.configured?'Saldo inicial definido; movimentos desde '+date(d.condominium_balance_settings[0].opening_on):'Saldo inicial não definido: apenas movimento líquido registado.'}</p><div class="cr-stats">${[['Saldo inicial',cash.opening],['Recebimentos',cash.received],['Despesas pagas',cash.spent],[cash.configured?'Saldo registado':'Movimento líquido',cash.closing],['Por pagar a fornecedores',cash.payable]].map(([k,v])=>`<div><strong>${cash.valid?money(v):'—'}</strong><span>${k}</span></div>`).join('')}</div></section>
 ${table('Despesas do condomínio',['Descrição','Categoria','Fornecedor','Vencimento','Valor','Estado','Pagamento','Fatura'],(d.condominium_expenses||[]).map(x=>[x.description,x.category,supplier(x.supplier_id),date(x.due_on),money(cents(x.amount)),x.status==='pending'?'Por pagar':label(x.status),date(x.paid_on),x.invoice_ref]))}
 ${table('Custos de serviços',['Serviço','Fornecedor','Valor por vencimento','Meses entre pagamentos','Próximo vencimento','Ativo'],(d.service_contracts||[]).map(x=>[x.title,supplier(x.supplier_id),money(cents(x.amount)),x.interval_months,date(x.next_due_on),x.active?'Sim':'Não']))}
