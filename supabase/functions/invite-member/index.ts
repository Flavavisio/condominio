...
export default {
  fetch: withSupabase({ auth: "user" }, async (req, ctx) => {
    ...
    if (condominiumId) {
      const { data: condo, error: condoError } = await ctx.supabase
        .from("condominiums")
    ...
    if (!inviteError && inviteData?.user) {
      ...
      try {
        targetUser = await findExistingUserByEmail(ctx.supabaseAdmin, email);
      } catch (error: any) {
    ...
    if (companyId) {
      const { error } = await ctx.supabaseAdmin
        .from("company_members")
        .upsert({ company_id: companyId, user_id: targetUser.id, role: companyRole, status: "active" }, { onConflict: "company_id,user_id" });
    ...
    if (condominiumId) {
      ...
      if (existing?.id) {
        const { error } = await ctx.supabaseAdmin
          .from("condominium_members")
          .update({ fraction_id: fractionId, member_role: memberRole, status: "active" })
          .eq("id", existing.id);
        if (error) return json({ error: error.message }, 400);
      } else {
        const { error } = await ctx.supabaseAdmin
          .from("condominium_members")
          .insert({ condominium_id: condominiumId, fraction_id: fractionId, user_id: targetUser.id, member_role: memberRole, status: "active" });
        if (error) return json({ error: error.message }, 400);
      }
    }
