...
export function shell(s,content,{resident=false,superAdmin=false,title=''}={}) {
  ...
  if (!resident && !superAdmin && s.companyMembers.some(m=>m.user_id===s.user?.id && m.role==='admin' && m.status==='active')) sidebarMenu.splice(1,0,['team','Equipa','users']);
...
export function condoCards(s) {return scopedCondos(s).length?`<div class="cf-condo-cards">${scopedCondos(s).slice(0,3).map((c,i)=>`<button data-condo="${e(c.id)}">${condoPicture(c,i)}<strong>${e(c.name)}</strong><small>${s.fractions.filter(f=>f.condominium_id===c.id).length||Number(c.fractions_count)||0} frações</small></button>`).join('')}</div>`:compactEmpty('Ainda não existem condomínios.');}
function updates(s) {
...
export function condoPicture(c,index=0,extra='') { return c.coverUrl ? `<img class="cf-building-photo cf-condo-cover ${extra}" src="${e(c.coverUrl)}" alt="Fotografia de ${e(c.name)}" loading="lazy">` : buildingPicture(index,extra); }
