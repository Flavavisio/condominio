...
export const brand = () => `<span class="cf-building-logo" aria-hidden="true"><svg viewBox="0 0 54 54"><path d="M4 46V23l12-6v30M16 47V7L31 1v48" fill="none" stroke="#0b3b5c" stroke-width="3"/><path d="m31 3 7 5v41l-7-1Z" fill="#0b3b5c"/><path d="m40 17 11 6v28l-11-2Z" fill="#227f99"/><path d="M3 48 31 46l21 6H3Z" fill="#1a5f7a"/><path d="M21 10v31" stroke="#b3d5e3" stroke-width="4"/></svg></span><strong>Condomínio Fácil</strong>`;
...
const pill = issue => `<span class="cf-status ${e(issue.status)}">${e(issueLabel(issue.status))}</span>`;
...
export function shell(s,content,{resident=false,superAdmin=false,title=''}={}) {
  ...
  const links=resident?menu.filter(([id])=>!['fractions','maintenance','suppliers','reports'].includes(id)):menu;
  return `<div class="app-shell mockup-shell ${resident?'cf-resident-shell':''}">
    <aside class="sidebar"><a class="cf-brand" href="#" data-view="dashboard" aria-label="Condomínio Fácil — Início">${brand()}</a>
      ${!resident?`<label class="cf-company-switch"><span>Empresa gestora</span><select id="companyFilter" aria-label="Empresa gestora">${s.companies.length>1?`<option value="">Todas as empresas</option>`:''}${s.companies.map(c=>`<option value="${e(c.id)}" ${s.dashboardCompanyId===c.id||(!s.dashboardCompanyId&&s.companies.length===1)?'selected':''}>${e(c.label||c.name)}</option>`).join('')}${!s.companies.length?'<option>Sem empresa associada</option>':''}</select></label>`:''}
      <nav aria-label="Menu principal">${links.map(args=>nav(s,...args)).join('')}</nav>
      <div class="cf-sidebar-bottom">${nav(s,'settings','Configurações','settings')}</div>
    </aside><button type="button" class="cf-menu-backdrop" aria-label="Fechar menu" id="closeSidebar"></button>
    <main class="main"><header class="topbar"><div class="cf-topbar-title"><button type="button" class="cf-menu-toggle icon-btn" id="menuToggle" aria-label="Abrir menu" aria-expanded="false">${icon('menu')}</button><h1>${e(title||labels[s.view]||'Visão geral')}</h1>${superAdmin?'<span class="eyebrow blue cf-role-context">Super Admin</span>':''}</div>
      <a class="cf-brand cf-mobile-brand" href="#" data-view="dashboard">${brand()}</a>
      <div class="cf-header-right"><form id="globalSearch" class="cf-search" role="search">${icon('search')}<input name="q" aria-label="Pesquisar" placeholder="Pesquisar..." value="${e(s.searchQuery||'')}" autocomplete="off"></form><div class="topbar-actions"></div><button class="cf-account" type="button" data-view="profile"><span class="avatar">${e(initials(nameFor(s)))}</span><span>${e(nameFor(s))}</span>${icon('down')}</button></div>
    ...
    ${resident?`<nav class="cf-mobile-nav" aria-label="Navegação do condómino">${[['dashboard','Início','home'],['operations','Ocorrências','tool'],['notices','Avisos','notice'],['profile','Perfil','user']].map(args=>nav(s,...args)).join('')}</nav>`:''}
...
export function agendaItems(s) {
  return [...scopedItems(s,'maintenance').filter(i=>i.scheduled_for&&!['done','cancelled'].includes(i.status)).map(i=>({...i,when:i.scheduled_for,tab:'maintenance',ic:'tool'})),...scopedItems(s,'obligations').filter(i=>i.active&&i.next_date).map(i=>({...i,when:i.next_date,tab:'obligations',ic:'shield'}))].sort((a,b)=>a.when.localeCompare(b.when));
}
export function agenda(s,limit=3) {
  const items=agendaItems(s).slice(0,limit);
  return items.length?`<div class="cf-agenda">${items.map(i=>{const dt=new Date(i.when.length===10?`${i.when}T12:00:00`:i.when);return `<button class="cf-agenda-item" data-condo="${e(i.condominium_id)}" data-tab="${i.tab}"><span class="cf-calendar-date"><strong>${dt.getDate()}</strong><small>${dt.toLocaleDateString('pt-PT',{month:'short'}).replace('.','').toUpperCase()}</small></span>${icon(i.ic)}<span><strong>${e(i.title)}</strong><small>${i.when.length>10?dt.toLocaleTimeString('pt-PT',{hour:'2-digit',minute:'2-digit'}):'Todo o dia'}</small><small class="cf-location">${icon('pin')}${e(condoName(s,i.condominium_id))}</small></span></button>`;}).join('')}</div>`:compactEmpty('Sem eventos agendados.');
}
...
function condoCards(s) {return scopedCondos(s).length?`<div class="cf-condo-cards">${scopedCondos(s).slice(0,3).map((c,i)=>`<button data-condo="${e(c.id)}">${buildingPicture(i)}<strong>${e(c.name)}</strong><small>${s.fractions.filter(f=>f.condominium_id===c.id).length||Number(c.fractions_count)||0} frações</small></button>`).join('')}</div>`:compactEmpty('Ainda não existem condomínios.');}
function updates(s) {
...
export function dashboard(s) {
  ...
  const metrics=[['Condomínios',condos.length,'buildings','blue'],['Frações',fractions,'users','green'],['Ocorrências abertas',open.length,'tool','amber'],['Assembleias agendadas','—','calendar','blue']];
  return `<section class="cf-welcome"><div><h2>Olá, ${e(firstName(s))}</h2><p>Tudo o que precisa de acompanhar, num só lugar.</p></div><div class="cf-welcome-actions">${filter(s)}<button class="primary-btn" data-new-issue>${icon('plus')}Nova ocorrência</button></div></section>
    <section class="cf-metrics">${metrics.map(([label,value,ic,tone])=>`<article class="cf-metric" ${label==='Assembleias agendadas'?'title="Módulo de assembleias ainda não disponível"':''}><span class="cf-square-icon ${tone}">${icon(ic)}</span><div><strong>${value}</strong><span>${label}</span></div></article>`).join('')}</section>
...
export function resident(s) {
  ...
  return `<div class="cf-resident-dashboard"><section class="cf-resident-hero"><div><h2>Olá, ${e(firstName(s))}</h2><p>${e(condo?.name||'O meu condomínio')}${fraction?` · Fração ${e(fraction.code)}`:''}</p>${memberships.length>1?`<select id="residentCondo" aria-label="O meu condomínio">${s.condominiums.filter(c=>memberships.some(m=>m.condominium_id===c.id)).map(c=>`<option value="${e(c.id)}" ${condo?.id===c.id?'selected':''}>${e(c.name)}</option>`).join('')}</select>`:''}</div><div class="cf-resident-building" role="img" aria-label="Imagem ilustrativa de edifício"></div></section>
      <button class="primary-btn cf-report" data-new-issue data-resident-condo="${e(condo?.id||'')}">${icon('tool')}Reportar avaria</button>
      <section class="cf-resident-shortcuts">${[['documents','Documentos','document'],['assemblies','Assembleias','users'],['votes','Votações','vote'],['reservations','Reservas','calendar']].map(([id,label,ic])=>`<button data-resident-route="${id}" data-resident-condo="${e(condo?.id||'')}">${icon(ic)}<span>${label}</span></button>`).join('')}</section>
      <article class="panel cf-my-issue"><div class="panel-head"><h2>A minha ocorrência</h2><button class="text-btn" data-view="operations" aria-label="Ver ocorrências">${icon('chevron')}</button></div>${mine?`<button class="cf-resident-issue" data-condo="${e(mine.condominium_id)}" data-tab="issues"><span class="cf-round-icon blue">${icon('drop')}</span><span><strong>${e(mine.title)}</strong>${pill(mine)}<small>${mine.status==='progress'?'O técnico está a tratar do problema.':mine.status==='resolved'?'O problema foi resolvido.':'Acompanhe aqui o estado do seu pedido.'}</small></span></button>`:compactEmpty('Ainda não reportou ocorrências.')}</article>
      <article class="panel cf-next-assembly"><div class="panel-head"><h2>Próxima assembleia</h2><button class="text-btn" data-view="assemblies" aria-label="Ver assembleias">${icon('chevron')}</button></div><div class="cf-assembly-empty">${icon('calendar')}<span>Assembleias ainda não disponíveis.</span></div><button class="primary-btn" data-view="assemblies">Ver convocatória</button></article></div>`;
}
